...
import {Button, Dialog, DialogActions, DialogContent, DialogTitle, Divider, FormControl, FormHelperText, InputLabel, MenuItem, Pagination, Select, TextareaAutosize} from "@mui/material"
...
export const QnaBoards = ({
  ...
  const boardsDetail = async (board: BoardControl, index: number) => {
    ...
    if (!user.is_admin && user.user_pk !== board.user_pk) {
...
const QnaBoardsForm = ({
  ...
  const {formState: {errors}, control} = boardForm
  ...
  const boardsAction = async () => {
    ...
    if (board.contents.replace(/(<([^>]+)>)/gi, "").length < 5) {
