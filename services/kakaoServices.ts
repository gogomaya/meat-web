...
export const kakaoServices = {
  accessToken: async (code: string, state: string): Promise<ResponseApi> => {
    try {
      ...
      url += `&client_id=${process.env.NEXT_PUBLIC_KAKAO_REST_API_KEY}`
      url += `&redirect_uri=${process.env.NEXT_PUBLIC_URL}${process.env.NEXT_PUBLIC_KAKAO_REDIRECT_URI}`
      url += `&client_secret=${process.env.NEXT_PUBLIC_KAKAO_CLIENT_SECRET}`
